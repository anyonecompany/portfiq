...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  bool _isUsingDefaultEtfs = false;

  void _loadRegisteredEtfs() {
    final box = Hive.box('settings');
    final stored = box.get('registered_etfs');
    final isDefault = stored == null || (stored is List && stored.isEmpty);
    final tickers = isDefault ? kDefaultEtfs : (stored as List).cast<String>();
    setState(() {
      _isUsingDefaultEtfs = isDefault;
      _registeredEtfs =
          tickers.map((t) => _RegisteredEtf(ticker: t, name: t)).toList();
    });
  }
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child:
                const Text('닫기', style: TextStyle(color: PortfiqTheme.accent)),
          ),
        ],
      ),
    );
